...
def minutes_until_departure(dep_time_str):
    now = datetime.now()
    dep_time = datetime.strptime(dep_time_str, "%H:%M").replace(year=now.year, month=now.month, day=now.day)
    delta = dep_time - now
...
def assign_group(schedule, adults, toddlers, wheelchair_count, group_size, group_id, confirmed=False, restricted_carriages=None):
    schedule = sorted(schedule, key=lambda x: datetime.strptime(x['departure_time'], "%H:%M"))
